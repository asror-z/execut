...
$sSource = 'd:\Develop\General\PHP\AppsIDE\Web\PhpStorm\Settings\Backups\'
$sDest = 'd:\Develop\General\PHP\AppsIDE\Web\PhpStorm\Settings\Backups\'
	
		_FZ_Robo ($sSource, $sDest)
...
Func _FZ_TeraCopy ($sSource, $sDest)

    _FZ_IsExist($sSource)
    DirCreate($sDest)
    ShellExecuteWait('tera', 'Copy "' & $sSource &'" "' & $sDest &'"  /OverwriteAll /NoClose')

    Return True
EndFunc   ;==>_FZ_TeraCopy


Func _FZ_DirIsEmpty($Dir)
    Local $Search, $i, $File
...
EndFunc   ;==>_FZ_DirIsEmpty



Func _FZ_FFS ($sSource, $sDest)

    _FZ_IsExist($sSource)

    ShellExecuteWait('ffs', $ffsapp & ' -DirPair "'&$sSource&'" "'&$sDest&'"', '', $SHEX_OPEN, @SW_HIDE	)
...
Func _FZ_Robo ($sSource, $sDest)

    _FZ_IsExist($sSource)
    ;	RunCmdRead('RoboCopy "'&$sSource&'" "'&$sDest&'" *.* /E /COPY:DAT /NOCOPY /MIR /A+:RHASCN /CREATE /MT:8 /R:5 /V')
	
	; "d:\Develop\General\PHP\AppsIDE\Web\PhpStorm\Settings\Backups\" "d:\Develop\General\PHP\AppsIDE\Web\PhpStorm\Settings\Backups\" *.* /E /COPY:DAT /NOCOPY /MIR /A+:RHASCN /CREATE /MT:8 /R:5 /V
	
	$cmd = '"'&$sSource&'" "'&$sDest&'" *.* /E /COPY:DAT /NOCOPY /MIR /A+:RHASCN /CREATE /MT:8 /R:5 /V'
    ; ShellExecuteWait('RoboCopy', $cmd, '', $SHEX_OPEN, @SW_HIDE	)
	
	RunCmdRead('RoboCopy ' & $cmd)
...
Func _FZ_TeraMove ($sSource, $sDest)

    _FZ_IsExist($sSource)
    DirCreate($sDest)
    ShellExecuteWait('tera', 'Move "' & $sSource &'" "' & $sDest &'"  /OverwriteAll /NoClose', '', $SHEX_OPEN, @SW_HIDE)

    Return True
EndFunc   ;==>_FZ_TeraMove



Func _FZ_CmdDelete ($sSource)

    _FZ_IsExist($sSource)

    ShellExecuteWait('cmd', '/c rmdir /s /q "' & $sSource & '"')

    Return True
EndFunc   ;==>_FZ_CmdDelete
...
Func _FZ_FfsBackup ($sSource, $sDest)
    ...
    Return True
EndFunc   ;==>_FZ_FfsBackup




Func _FZ_FfsSync ($sSource, $sDest)
