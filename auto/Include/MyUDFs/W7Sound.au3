...
Func _W7S_VolumeToggle ()
    

    _OpenVolControl(1)
    
    If Not $bIsMuted Then
        $iOldMasterVolume = _GetMasterVolume()
        _SetMasterVolume(0)
        $bIsMuted = True
    Else
        _SetMasterVolume($iOldMasterVolume)
        $bIsMuted = False
    EndIf
...
Func T_Sample()
    ...
    For $i = 2 To $aProgs[0][0]
        ...
        _Log($aProgs[$i][0] & ' - ' & _GetVolume($aProgs[$i][2]) & @CRLF)
    Next
    Sleep(840)
    For $i = 2 To $aProgs[0][0]
